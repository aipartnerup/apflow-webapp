...
test('reports demo init status as unavailable when auto-login is not configured', async () => {
  const { AIPartnerUpFlowClient, getCalls } = loadClient(async () => {
  ...
  await assert.rejects(
    () => client.checkDemoInitStatus(),
    /Demo task initialization is not available/
  );
